  ...
  return (
    <FloatingActionPanelContext.Provider value={floatingPanelLogic}>
      <MotionConfig transition={TRANSITION}>
        <div className={cn("relative", className)}>{children(floatingPanelLogic)}</div>
      </MotionConfig>
...
export function FloatingActionPanelTrigger({
  ...
  if (!ctx) {
    throw new Error("FloatingActionPanelTrigger must be used inside FloatingActionPanelRoot");
  }
  const { openPanel, uniqueId, setTitle } = ctx;
...
export function FloatingActionPanelContent({
  ...
  const ctx = React.useContext(FloatingActionPanelContext);
  if (!ctx) {
    throw new Error("FloatingActionPanelContent must be used inside FloatingActionPanelRoot");
  }

  const { isOpen, closePanel, uniqueId, triggerRect, title, mode } = ctx;
...
export function FloatingActionPanelForm({
  ...
  const ctx = React.useContext(FloatingActionPanelContext);
  if (!ctx) {
    throw new Error("FloatingActionPanelForm must be used inside FloatingActionPanelRoot");
  }

  const { note, closePanel } = ctx;
...
export function FloatingActionPanelTextarea({
  ...
  const ctx = React.useContext(FloatingActionPanelContext);
  if (!ctx) {
    throw new Error("FloatingActionPanelTextarea must be used inside FloatingActionPanelRoot");
  }

  const { note, setNote } = ctx;
