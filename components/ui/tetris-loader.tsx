...
export default function TetrisLoading({
  ...
  const [grid, setGrid] = useState<Cell[][]>(() =>
    Array(config.gridHeight)
      .fill(null)
      .map(() => Array(config.gridWidth).fill(null).map(() => ({ filled: false, color: "" })))
  );
      ...
      <div key={rowIndex} className="flex">
        {row.map((cell, colIndex) => (
          <div
            ...
            className={`${config.cellSize} border border-gray-300 transition-all duration-100 dark:border-gray-600 ${
              cell.filled ? `${cell.color} scale-100` : "bg-white scale-95 dark:bg-black"
  ...
  return (
    <div>
      <div className="mb-6">
        <div className={`border-2 border-gray-800 bg-white transition-colors dark:border-gray-200 dark:bg-black ${config.padding}`}>
          {renderGrid()}
        </div>
      </div>
      ...
      {showLoadingText && (
        <div className="text-center">
          <p className="font-medium text-black transition-colors dark:text-white">{loadingText}</p>
        </div>
      )}
    </div>
  );
